...
function tableSortJs(testingTableSortJS = false, domDocumentWindow = document) {
    ...
    function makeEachColumnSortable(th, columnIndex, tableBody, sortableTable) {
        ...
        function getTableData(tableProperties) {
            ...
            for (let [i, tr] of tableRows.entries()) {
                let tdTextContent = tr.querySelectorAll("td").item(colSpanData[columnIndex] === 1 ? colSpanSum[columnIndex] - 1 : colSpanSum[columnIndex] - colSpanData[columnIndex]).textContent;
            ...
            function sortAscending(a, b) {
                if (a.includes(`${fillValue}#`)) {
                    ...
                    return a.localeCompare(b, navigator.languages[0] || navigator.language, { numeric: !isAlphaSort, ignorePunctuation: !isPunctSort });
                }
            }
        ...
        function updateTable(tableProperties) {
            ...
            for (let [i, tr] of tableRows.entries()) {
                if (isFileSize) {
                    ...
                    const fileSizeInBytesText = tr.querySelectorAll("td").item(columnIndex).textContent;
                    ...
                    for (let i = 0; i < prefixes.length; ++i) {
                        ...
                        if (fileSize < nextPrefixMultiplier) {
                            ...
                            fileSizeInBytesHTML = fileSizeInBytesHTML.replace(fileSizeInBytesText, `${(fileSize / prefixMultiplier).toFixed(2)} ${prefixes[i]}B`);
                            replaced = true;
                            break;
                        }
                    }
